...
const isSubListStart = (item) =>
  item[SUBLIST_INDICATOR_INDEX] &&
  item[SUBLIST_INDICATOR_INDEX].trim() === SUBLIST_INDICATOR;
...
const csv2JsonArray = (text) => {
  const helpArray = [];
  splitLines(text)
    .filter((_, index) => !LINES_TO_IGNORE.includes(index))
    .forEach((lineText) => helpArray.push(splitLine(lineText)));
